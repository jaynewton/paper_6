...
load("F:/我的论文/第五篇/RData/da_max_m.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_all_m <- da_all_m[,.(ym,SecCode,ret_e,size)]
da_m <- merge(da_all_m,da_max_m,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(max_ret_1),]
  #da_sub <- da_sub[order(max_ret_2),]
  #da_sub <- da_sub[order(max_ret_3),]
  #da_sub <- da_sub[order(max_ret_4),]
  #da_sub <- da_sub[order(max_ret_5),]
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(max_ret_1)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(max_ret_1,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(max_ret_2)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(max_ret_2,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(max_ret_3)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(max_ret_3,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(max_ret_4)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(max_ret_4,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(max_ret_5)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(max_ret_5,size)]
  }
}
