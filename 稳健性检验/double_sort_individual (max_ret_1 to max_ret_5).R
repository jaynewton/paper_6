...
da_m <- merge(da_m,da_max_m,by=c("ym","SecCode")) 
...
for (i in 1:length(ym_index)) {
  ...
  for (j in 1:k) {
    ...
    da_ss <- da_ss[order(max_ret_1),] # variable of interest
    #da_ss <- da_ss[order(max_ret_2),]
    #da_ss <- da_ss[order(max_ret_3),]
    #da_ss <- da_ss[order(max_ret_4),]
    #da_ss <- da_ss[order(max_ret_5),]
