...
load("F:/我的论文/第五篇/主代码/individual investor preference/RData/da_breadth_5y.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_all_m <- da_all_m[,.(ym,SecCode,ret_e,size)]
da_m <- merge(da_all_m,da_breadth_5y,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(breadth)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(breadth,size)]
  }
}
