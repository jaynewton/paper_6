...
load("F:/我的论文/第五篇/主代码/individual investor preference/RData/da_sk_tail_6m.RData")
load("F:/我的论文/第五篇/RData/FF3F_A_nm.RData")

da_all_m <- da_all_m[,.(ym,SecCode,ret_e,size)]
da_m <- merge(da_all_m,da_sk_tail_6m,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(sk_1),]
  #da_sub <- da_sub[order(sk_5),]
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(sk_1)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(sk_1,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(sk_2)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(sk_2,size)]
  }
}
