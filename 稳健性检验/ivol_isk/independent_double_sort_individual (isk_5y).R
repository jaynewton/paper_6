...
da_m <- merge(da_m,da_ivol_5y,by=c("ym","SecCode")) 

da_m[,group_isk:=ifelse(isk<=quantile(isk,0.2),1,
                        ifelse(isk<=quantile(isk,0.4),2,
                               ifelse(isk<=quantile(isk,0.6),3,
                                      ifelse(isk<=quantile(isk,0.8),4,
                                             5)))),by=ym]
...
for (i in 1:length(ym_index)) {
  da_ret_e <- da_m[ym==ym_index[i],.(ret_e=mean(ret_e)),keyby=.(group_isk,group_individual)]
  #da_ret_e <- da_m[ym==ym_index[i],.(ret_e=weighted.mean(ret_e,size)),keyby=.(group_isk,group_individual)]
  ret_p[i,,] <- as.matrix(dcast(da_ret_e, group_isk ~ group_individual, value.var="ret_e")[,-"group_isk"])
}
...
for (i in 1:length(ym_index)) {
  for (p in 1:k) {
    for (j in 1:k) {
      ret_p[i,p,j] <- da_m[ym==ym_index[i] & group_isk==p & group_individual==j,mean(ret_e)]
      #ret_p[i,p,j] <- da_m[ym==ym_index[i] & group_isk==p & group_individual==j,weighted.mean(ret_e,size)]
    }
  }
}
