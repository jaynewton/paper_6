...
da_m <- merge(da_all_m,da_tsk_3m,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(tsk)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(tsk,size)]
  }
}
