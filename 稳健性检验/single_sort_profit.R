...
da_m <- merge(da_all_m[,.(ym,SecCode,ret_e,size)],da_profit_m,by=c("ym","SecCode"))
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(eps),]
  #da_sub <- da_sub[order(roe),]
  #da_sub <- da_sub[order(opps),]
  ...
  for (j in 1:k) {
    ...
    vari_level[i,j] <- da_sub[group_n==j,mean(eps)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(eps,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(roe)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(roe,size)]
    #vari_level[i,j] <- da_sub[group_n==j,mean(opps)]
    #vari_level[i,j] <- da_sub[group_n==j,weighted.mean(opps,size)]
  }
}
