...
load("F:/我的论文/第五篇/主代码/individual investor preference/RData/da_tsk_6m.RData")
...
da_m <- merge(da_m,da_tsk_6m,by=c("ym","SecCode")) 

da_m[,group_tsk:=ifelse(tsk<=quantile(tsk,0.2),1,
                        ifelse(tsk<=quantile(tsk,0.4),2,
                               ifelse(tsk<=quantile(tsk,0.6),3,
                                      ifelse(tsk<=quantile(tsk,0.8),4,
                                             5)))),by=ym]
...
for (i in 1:length(ym_index)) {
  da_ret_e <- da_m[ym==ym_index[i],.(ret_e=mean(ret_e)),keyby=.(group_tsk,group_individual)]
  #da_ret_e <- da_m[ym==ym_index[i],.(ret_e=weighted.mean(ret_e,size)),keyby=.(group_tsk,group_individual)]
  ret_p[i,,] <- as.matrix(dcast(da_ret_e, group_tsk ~ group_individual, value.var="ret_e")[,-"group_tsk"])
}
...
for (i in 1:length(ym_index)) {
  for (p in 1:k) {
    for (j in 1:k) {
      ret_p[i,p,j] <- da_m[ym==ym_index[i] & group_tsk==p & group_individual==j,mean(ret_e)]
      #ret_p[i,p,j] <- da_m[ym==ym_index[i] & group_tsk==p & group_individual==j,weighted.mean(ret_e,size)]
    }
  }
}
